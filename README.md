...
```java
        DefaultRestClient webClient = new DefaultRestClient("http://localhost");
        ResponseEntity<String> response = webClient.get("/api/status", new ParameterizedTypeReference<String>() {});
```
